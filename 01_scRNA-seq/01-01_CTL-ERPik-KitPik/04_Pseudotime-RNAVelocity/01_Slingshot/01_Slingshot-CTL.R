#####################################################################################
# Slingshot trajectory inference on CTL epithelial cells and pseudotime-GAM dynamics
#
# Description:
#   Runs Slingshot trajectory inference on CTL epithelial cells (non-proliferative)
...
#     2. Subset to CTL sample and remove proliferating cells ("Prol")
...
seuset <- seuset[, seuset$sample == "CTL"]

## Configuration
fig_path <- file.path("figures_CTL")
dir.create(fig_path, recursive = TRUE)
res_path <- file.path("results_CTL")
dir.create(res_path, recursive = TRUE)

sling_fig_path <- file.path(fig_path, "slingshot-PCA_CTL")

#####################################################################################
# Slingshot trajectory inference on Kit_Pik epithelial cells and pseudotime-GAM dynamics
#
# Description:
#   Runs Slingshot trajectory inference on Kit_Pik epithelial cells (non-proliferative)
...
#     2. Subset to Kit_Pik sample and remove proliferating cells ("Prol")
...
seuset <- seuset[, seuset$sample == "Kit_Pik"]

## Configuration
fig_path <- file.path("figures_Kit_Pik")
dir.create(fig_path, recursive = TRUE)
res_path <- file.path("results_Kit_Pik")
dir.create(res_path, recursive = TRUE)

sling_fig_path <- file.path(fig_path, "slingshot-PCA_Kit_Pik")
...
### To check whether the cell types are correKit_Piky embedded

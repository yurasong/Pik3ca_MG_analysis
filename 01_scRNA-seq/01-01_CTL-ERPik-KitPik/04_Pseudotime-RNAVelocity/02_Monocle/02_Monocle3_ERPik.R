#####################################################################################
# Monocle3 trajectory inference and pseudotime visualization (ER Pik Yfp epithelial subset)
#
# Description:
...
#   counts, and dimensional reduction (PCA→UMAP) is computed. A ER Pik Yfp-only subset
...
#     5. Subset CDS to ER Pik Yfp cells for trajectory learning
...
#   - The trajectory is learned on ER Pik Yfp cells only (sample == "ER_Pik") to define
...
cds_erp <- cds[, colData(cds)$sample == "ER_Pik"]
...
plot_cells(cds_erp,
  ...
  scale_color_manual(values=c("#FFA500", "#32CD32", "#2F4F4F", 
                              "#800000", "#F08080", "#BDB76B",  "#1E90FF")) + 

#####################################################################################
# File: 01_AUC_EarlyHY.R
#
# Description:
#   1. Load processed Seurat object from Wu et al. (Nat Genet, 2021)
#   2. Build AUCell rankings and calculate AUC for the Early_HY_signature gene set
...
#   - 01_Signature/early_HY.txt      : List of genes defining the Early HY signature
#
# Outputs:
...
# Geneset allocation
early_hy <- read.delim("01_Signature/early_HY.txt", h=F)
early_hy <- early_hy$V1
early_hy <- unique(early_hy)

geneSets <- GeneSet(early_hy, setName="Early_HY_signature")
